...
def h_X(q0_k, q1_k, q2_k, q3_k, w0_k, w1_k, w2_k,Bref):
...
def F(q0_k, q1_k, q2_k, q3_k, w0_k, w1_k, w2_k):
    ...
    F_k = np.array([F1,F2,F3,F4,F5,F6,F7]) #JACOBIANO DEL VECTOR ESTADO
...
def Q(noise_mag, noise_gyros):
    ...
    Q_k = np.array([Q1,Q2,Q3,Q4,Q5,Q6,Q7]) #MATRIZ DE COVARIANZA DEL RUIDO DE SENSORES
...
#     deltaq_norm = deltaq/np.linalg.norm(deltaq) #POR AHORA SOLO NORMALIZAR 
#     q_k = np.array(q_k)
#     deltaq_norm = np.array(deltaq_norm)
...
#     return q_posteriori #SE DICE QUE EL q_posteriori DEPENDE DE LA MULTIPLICACION DEL CUATERNION (CONSULTAR NO LO ENTIENDO)
...
if len(tle_lines) < 2:
    ...
    P_ki = np.identity(7) #P para iniciar el kiltro de kalman
    ...
    for i in range(len(t_aux)-1):
        ...
        F_k = F(Xk[0],Xk[1],Xk[2],Xk[3],Xk[4],Xk[5],Xk[6])
        Q_k = Q(ruido_bad, noise_rms_bad)
        P_k_priori = P_k_prior(F_k, P_ki, Q_k)
        H_k = H(Xk[0],Xk[1],Xk[2],Xk[3],Xk[4],Xk[5],Xk[6], Bs[i,:]) #B_s IGRF
        K_k = k_kalman(P_k_priori, H_k)
        B_mod = h_X(Xk[0],Xk[1],Xk[2],Xk[3],Xk[4],Xk[5],Xk[6],Bs[i,:])
        ...
        q_k_pos = np.array([q_k[0]+d_q[0], q_k[1]+d_q[1], q_k[2]+d_q[2], q_k[3]+d_q[3]])
        q_k_pos_norm = q_k_pos / np.linalg.norm(q_k_pos)
        w_k_pos = np.array([w_k[0]+d_w[0], w_k[1]+d_w[1], w_k[2]+d_w[2]])
        P_k_pos= P_posteriori(K_k,H_k,P_k_priori)
        
        
        
        q_k00.append(float(q_k_pos_norm[0]))
        q_k10.append(float(q_k_pos_norm[1]))
        q_k20.append(float(q_k_pos_norm[2]))
        q_k30.append(float(q_k_pos_norm[3]))
